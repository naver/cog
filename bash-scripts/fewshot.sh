...
# Bash script to train linear logistic regression classifiers
# with different number of training samples per class and different seeds
...
# all the few-shot experiments on the pre-extracted features.
# (For advanced users: Experiments in the for loops are independent, meaning that you can
...
# If you haven't done it already, please see README.md and bash-script/extract_features.sh.
...
# Few-shot experiments with different number of training samples per class
for n_shot in 1 2 4 8 16 32 64 128; do

    # For loop to train 5 classifiers with different seeds
    for seed in 22 37 45 77 93; do

        # Lets make sure that
        # we save the logs for the experiments with different <n_shot> under different directories and
        # put the experiments with different seeds under the same roof.
        # If you provided <models_root_dir> and left <output_dir>=None
        # don't worry, logreg.py will handle the folder name
        output_dir_="None"
        [[ "${output_dir}" != "None" ]] && output_dir_=${output_dir}_N${n_shot}/seed-${seed}

        echo "logreg n-shot: ${n_shot} | seed: ${seed}"
        python logreg.py \
...
# Print the average score for each few-shot experiment
for n_shot in 1 2 4 8 16 32 64 128; do
    echo "--------------------------------------------------"
    echo "Few-shot results for N = ${n_shot}"
    echo "--------------------------------------------------"

    output_dir_="None"
    [[ "${output_dir}" != "None" ]] && output_dir_=${output_dir}_N${n_shot}
    python print_results.py \
        --logreg_root_dir=${output_dir_} \
        --models_root_dir=${models_root_dir} \
        --model=${model_name} \
        --dataset=${dataset} \
        CLF.N_SHOT ${n_shot}
